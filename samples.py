...
pre_trans_obj = pre_trans_obj = utils.generate_pseudo_bunny(100)
...
inlier_mask, Y = utils.apply_outlier_noise(Y, utils.generate_spherical_noise, percentage=noise_level, magnitude=5.0)
...
utils.plot_shapes([X, Y[inlier_mask == 1], Y[inlier_mask == 0]],
                  colorscale=['#0074D9', '#2ECC40', '#FFDC00'],
                  legends=["original object", "inliers", "outliers"],
                  title=f"original shape with noise {noise_level}%")
...
utils.plot_shapes([X, Y[inlier_mask == 1], Y[inlier_mask == 0], ICP_obj],
                  colorscale=['#0074D9', '#2ECC40', '#FFDC00', '#FF4136'],
                  legends=["original object", "inliers", "outliers", "ICP"],
                  title=f"ICP transform with noise:{noise_level}%")
...
utils.plot_shapes([X, Y[inlier_mask == 1], Y[inlier_mask == 0], GM_obj],
                  colorscale=['#0074D9', '#2ECC40', '#FFDC00', '#FF4136'],
                  legends=["original object", "inliers", "outliers", "ICP"],
                  title=f"GNC-GM transform with noise:{noise_level}%")
...
utils.plot_shapes([X, Y[inlier_mask == 1], Y[inlier_mask == 0], TLS_obj],
                  colorscale=['#0074D9', '#2ECC40', '#FFDC00', '#FF4136'],
                  legends=["original object", "inliers", "outliers", "ICP"],
                  title=f"GNC-TLS transform with noise:{noise_level}%")
