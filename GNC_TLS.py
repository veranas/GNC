...
def GNC_TLS(source_points, target_points, inlier_mask, true_T, c_bar=1.0, max_outer_iterations=50, max_inner_iterations=10,
           tolerance=1e-6):
    """
    ...
    # Log data
    data.append([error_inlier, 0, 0, 0.0, cumulative_transformation.clone(), MSE_err(true_T, cumulative_transformation)])
    ...
    ###### FOR GNC-TLS ##########
    mu_init = (c_bar ** 2) / (2 * rmax - c_bar ** 2)
    ...
    for i in range(max_outer_iterations):
        ...
        # update weights
        w = utils.update_MSE_weights_TLS(cumulative_transformation, w, c_bar, mu, source_points, target_points)
        w = w.reshape(-1, 1)

        total_weight_calcs += 1  # count number of weight calculations

        if torch.all(w == 0.0):
            print("GNC_TLC failed to converge")
            break
        ...
        cost = torch.mean(torch.sum((target_points - current_source) ** 2, dim=1))
        #print(f"cost: {cost}")
